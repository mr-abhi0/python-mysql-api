...
@app.route(f"{route_prefix}/city", methods=['GET'])
def getdata():
    try:
        countrycode = request.args.get('country', default='IND', type=str)
        query = f"SELECT * FROM testdb.cities "
...
## /api/v1/postoffice?country=IND
@app.route(f"{route_prefix}/postoffice", methods=['GET'])
def postoffice():
    try:
        # countrycode = request.args.get('country', default='IND', type=str)
        query = f"SELECT * FROM testdb.post_office "
        records = db.run_query(query=query)
        response = get_response_msg(records, HTTPStatus.OK)
        db.close_connection()
        return response
...
## /api/v1/getcitycodes
@app.route(f"{route_prefix}/country", methods=['GET'])
def country():
    try:
        query = f"SELECT * FROM testdb.country_list"
        records = db.run_query(query=query)
        response = get_response_msg(records,  HTTPStatus.OK)
        db.close_connection()
        return response
